...
type BadgeColor = keyof typeof _colors
type MultiDayPosition = keyof typeof _multiDayPositions
...
export function MonthEventBadge({event, cellDate, eventCurrentDay, eventTotalDays, className, position: propPosition}: IProps) {
	...
	const eventBadgeClasses = `${_base} ${_colors[color || "blue-dot"]} ${_multiDayPositions[position]} ${className}`
